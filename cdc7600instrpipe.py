...
"""
    Functions for CDC7600System that are 
...
def createDesc(self, instr):
    ...
    if (instr.category == "FETCH"):
        ...
        # Use fuunctional unit for description
        instr.instrDesc = instr.catDesc + " " +\
                          instr.instrRegs['leftOp'] \
...
def checkDataDepend(self, instr):
    ...
    if instr.operator is not None:
        # If operation instruction, check each
        # instruction involved to make sure times are correct.
...
def performArithmetic(self, instr):
    """
    Calculates the equation output from each of the
     instruction managers in the final output instruction.
    :param self: CDC7600System
...
def generateTimes(self, instr):
    ...
    # Index by separate counter, instruction list indexes
    # comp operations incorrectly for some reason
    ...
    if instr.operator is not None and \
        ...
        instr.timeDict['startTime'] =\
            instr.instrManager.instrDict['assign2'].timeDict['fetchTime']
    ...
    instr.timeDict['resultTime'] = \
        instr.timeDict['startTime'] + funcTime
    ...
    if instr.category == "FETCH" or instr.category == "STORE":
        instr.timeDict['unitReadyTime']\
            = instr.timeDict['startTime'] + self.segTime['INCR']
    else:
        instr.timeDict['unitReadyTime'] \
            = instr.timeDict['startTime'] + self.segTime[instr.category]

    if instr.category == "FETCH":
        instr.timeDict['fetchTime'] \
            = instr.timeDict['resultTime'] + self.fetchStoreWait
    elif instr.category == "STORE":
