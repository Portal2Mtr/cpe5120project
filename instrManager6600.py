...
class instrManager():
    ...
    def __init__(self,compInstr,manageType,system):
        ...
        :param compInstr: Complex instruciton
        to refer to the manager by.
        :param manageType: Type of manager
        :param system: CDC 6600/7600 system.
        """
        # Manager types = ["SCALAR",
        # "SQUARE","CONSTANT","OUTPUT","OPERATIONS"]
        ...
        self.instrDict = {'assign1':None,
                          ...
                          'square':None,
                          'output':None}
        # Dict of managment instructions list
        # indices for cdc6600system instrList
    ...
    def getOutputIdx(self):
        """
        Gets the output register for a
         given type of input instruction.
    ...
    def getInstrOrder(self):
        """
        Returns a list of each instruction
        ordered in the manager's dictionary.
    ...
    def calcOutput(self,outputVal):
        """
        Set the output of the manager's
        instruction dictionary to a value.
    ...
    def getOutputVal(self):
        """
        Get the output value of this
         manager's instruction dictionary.
        :return:
        """
    ...
    def assignIdx(self,instr,idx):
        """
        Assign an instruction in the
        manager a given index.
        :param instr: Input instruction
    ...
    def genOpEqn(self,instr):
        ...
        if instr.operator == '+': # Check if
            # last addition, if so output to X7 if x6 is full

            if self.system.instrList[-2] is instr:
                # Last addition
                if self.system.opRegIdx > 1: # Assignment
                    # for x6 has passed
                    opAddr = 'X7'
            ...
            # Check if this is scalar multiple,
            # if so reuse assignment register
            if self.manageType == "SCALAR":
                reuseMem = 'X' + \
                           self.instrDict['assi' \
                            'gn2'].instrRegs['result'][-1]
                opAddr = reuseMem
            else:
                opAddr = self.system.opRegsList[self.system.opRegIdx]
        ...
        leftAddr = self.system.instrList[
            instr.leftOpIdx].instrRegs['result']
        rightAddr = self.system.instrList[
            instr.rightOpIdx].instrRegs['result']
    ...
    def linkInstr(self,instr):
        """
         Generate left/right idxs from
          instruction list for equation generation from within manager
        :param instr: Input instruction
        :return:
        """
        ...
        elif workKey == 'scalMult' and \
                self.manageType == 'SQUARE':
    ...
    def updateCompOpIdxs(self,compOpList):
        ...
        instruction index in the instruction
         list for proper instruction equation
        generation.
        :param compOpList: List of
        operations using the complex instructions.
        """
